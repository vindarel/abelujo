...
./manage.py import_csv_with_dilicom -i path/to.csv [-s shelf_id]
"""
...
class Command(BaseCommand):
    ...
    def run(self, *args, **options):
        ...
        """
        WARN_MSG = "***** This script sets the quantities instead of adding them. *****"
        print(WARN_MSG)
        ...
        if len(bklist) != len(isbns):
            ...
            self.stdout.write("--- isbns not found: {}".format(list(set(isbns) - set(bklist))))
        ...
        for i, bk in enumerate(bklist):
            ...
            dist = None
            if bk.get('distributor_gln'):
                dist, _dist_created = Distributor.objects.get_or_create(gln=bk.get('distributor_gln'))
                bk['distributor'] = dist
